...
#         return redirect('pages/forgot-password')
...
def Home(request):
    trending_games = ProductDetails.objects.filter(game_id__range=(1, 4))
    most_played = ProductDetails.objects.filter(game_id__range=(5, 10))
...
def auth_page(request):
    if request.method == "POST":
        ...
        if form_type == "signup":
            ...
            user.save()
            return render(request, "pages/login.html", {"success": "."})
